...
import type { UserHandle } from './world'
...
/**
 * Meldet den Nutzer an und legt die Session in einem frischen Cookie-Speicher
 * ab — im **selben Format**, das `@supabase/ssr` in der App schreibt.
 *
...
export async function signedInStore(user: UserHandle): Promise<FakeCookieStore> {
  ...
  if (error) throw new Error(`Anmeldung fehlgeschlagen für ${user.email}: ${error.message}`)
