...
Env-Vars: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY`, `NEXT_PUBLIC_SITE_URL`, `SIGNUP_INVITE_CODE` (Registrierung; **leer oder fehlend = Registrierung geschlossen**), `RESEND_API_KEY` + `GUEST_MAIL_FROM` (Gast-Zugang per Mail; **fehlt eines, bietet die Oberfläche den Versand gar nicht erst an** — der Druckweg bleibt davon unberührt), `ALLOW_TEST_ACCOUNTS=1` (**vorübergehend**, siehe unten; in Produktion nicht setzen).
...
- [Sessions/2026-09-04_Produktions-Testlauf.md](Sessions/2026-09-04_Produktions-Testlauf.md) — **HIER STARTEN.** Systematischer Testlauf der Implementierung vom 03.09. **in Produktion**, Claude fährt Chrome: alle acht Blöcke bestanden, neun Befunde, acht am selben Tag behoben (Dark-Mode-Tints an der Wurzel, `stays.checked_out_by`, Stale-Timeout als `clean_aborted` festgeschrieben, Aushänge unter „Gäste-Zugang", Namensauflösung über Akteur-IDs). Enthält Werkzeug-Notizen zum Browser-Fahren auf diesem Rechner und die Liste „Nicht durchgeführt". Der Plan selbst: [Testplan-Nachlauf-2026-09-03.md](Sessions/Testplan-Nachlauf-2026-09-03.md).
