...
Env-Vars: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY`, `NEXT_PUBLIC_SITE_URL`, `SIGNUP_INVITE_CODE` (Registrierung; **leer oder fehlend = Registrierung geschlossen**), `RESEND_API_KEY` + `GUEST_MAIL_FROM` (Gast-Zugang per Mail; **fehlt eines, bietet die Oberfläche den Versand gar nicht erst an** — der Druckweg bleibt davon unberührt).
