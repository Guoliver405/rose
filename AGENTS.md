...
- **Admin-Navigation** (25.07.2026): Nav = Übersicht | Bestellungen | Einstellungen. `/admin/einstellungen` ist ein rollenabhängiger Kachel-Hub: Admin sieht Hotel & Regeln (`/einstellungen/hotel`), Zimmer, Personal, Services, QR-Aushänge, Passwort (`/einstellungen/passwort`) und (temporär) Test-Szenario (`/einstellungen/test`); Rezeption nur Aushänge, Personal-Karten, Passwort. Die Setup-Routen selbst (`/admin/zimmer` usw.) und ihre Guards sind unverändert.
